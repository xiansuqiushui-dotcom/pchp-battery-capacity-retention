...
def make_figure(summary: pd.DataFrame, auc: pd.DataFrame, chinese: bool, stem: str) -> None:
    ...
    if chinese:
        ...
        xlab_a = "损害预算 $\\delta$（容量保持率百分点）"
        ylab_a = "PCHP − 精确预算可行偏移\nMAE（百分点）"
        better = "下方区域表示 PCHP 更优"
        main_label = "正文预算\n$\\delta=1$ 个百分点"
    else:
        title_a = "A  Candidate-information gain across harm budgets"
        xlab_a = "Harm budget $\\delta$ (capacity-retention percentage points)"
        ylab_a = "PCHP − exact budget-feasible offset\nMAE (percentage points)"
