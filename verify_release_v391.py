"""Portable end-to-end verifier for the V391 manuscript-alignment release."""
...
MANIFEST = ROOT / "manifest_v391.json"
EXCLUDED_NAMES = {MANIFEST.name, "verification_receipt_v391.json"}
...
def verify_manifest(passed: list[str]) -> None:
    ...
    check(manifest["version"] == "v391", "manifest version", passed)
...
def verify_manuscript(passed: list[str]) -> None:
    ...
    check(title in en, "final English title", passed)
    check("跨域锂离子电池容量保持率估计的风险受控在线更新" in zh and "前缀因果投影方法" in zh, "final Chinese title", passed)
    ...
    check(
        "label-free deployment cannot control model-update risk" in en
        and "无标签部署无法控制模型更新风险" in zh,
        "bounded bilingual novelty statement",
        passed,
    )
    check(
        title in " ".join(readme.split())
        and title in citation
        and 'version: "v391"' in citation,
        "repository metadata matches V391 manuscript",
        passed,
    )
...
def main() -> int:
    ...
    if args.write_receipt:
        path = ROOT / "verification_receipt_v391.json"
        with path.open("w", encoding="utf-8", newline="\n") as stream:
            stream.write(json.dumps(receipt, indent=2, ensure_ascii=False) + "\n")
        print(f"Wrote {path.name}")
    print(f"V391 RELEASE VERIFICATION PASSED: {len(passed)} checks")
    return 0
