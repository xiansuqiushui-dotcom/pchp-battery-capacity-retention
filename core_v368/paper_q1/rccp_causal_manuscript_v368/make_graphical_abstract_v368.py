...
def build_graphical_abstract(output_dir: Path) -> tuple[Path, Path]:
    ...
    panel_header(ax, 1.2, 1, "Operational SOH gap")
    ...
    ax.text(15.8, 29.1, "issued SOH", ha="center", color=NAVY, fontsize=9.8, fontweight="bold")
    ...
    ax.text(
        65.45,
        4.15,
        r"Every hidden outcome: $|p_t-y|-|b_t-y|\leq\delta$",
        ha="center",
        va="center",
        color=INK,
        fontsize=10.5,
    ...
    panel_header(ax, 92.0, 3, "Audited evidence")
    metric_card(ax, 35.0, BLUE, "Nested cross-domain", "12 dataset domains", r"11/12 improved  |  $\Delta$MAE $-0.00486$")
    metric_card(ax, 24.3, TEAL, "Outcome-blind external confirmation", "45 held-out eligible cells", r"45/45 improved  |  $\Delta$MAE $-0.00986$")
    metric_card(ax, 13.6, CORAL, "Unprotected opportunity cost", "stronger comparator", "12/12 budget violations")
    box(ax, 93.3, 3.0, 34.0, 7.4, face=PALE_GOLD, edge="#E4CA7A", radius=0.7)
    ax.text(110.3, 7.5, "Risk-bounded SOH updates", ha="center", color=INK, fontsize=10.6, fontweight="bold")
    ax.text(110.3, 5.0, "before reference capacity is available", ha="center", color=MUTED, fontsize=9.2)

    output_dir.mkdir(parents=True, exist_ok=True)
    png = output_dir / "graphical_abstract_applied_energy_v368.png"
    pdf = output_dir / "graphical_abstract_applied_energy_v368.pdf"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate the V368 Applied Energy graphical abstract.")
